...
node scripts/issue-decision-router.mjs \
  ...
  --schedule-keyword AI \
  --gov24-keyword 보육
node scripts/issue-onepager.mjs \
  --topic 공급망 \
  --policy-query 조달청 \
  --law-query 정부조직법 \
  --schedule-keyword AI \
...
node scripts/issue-action-queue.mjs \
  ...
  --schedule-keyword AI \
  --gov24-keyword 보육
node scripts/issue-geo-context.mjs \
  --topic 공급망 \
  --policy-query 조달청 \
  --law-query 정부조직법 \
  --format json
node scripts/issue-casefile.mjs \
  --topic 공급망 \
  --policy-query 조달청 \
  --law-query 정부조직법 \
  --schedule-keyword AI \
...
node scripts/issue-workflow.mjs \
  ...
  --schedule-keyword AI \
  --gov24-keyword 보육
node scripts/issue-ops-board.mjs \
  --root out/issue-casefiles
node scripts/issue-regression-check.mjs \
  --root out/issue-casefiles \
  --topic 공급망
node scripts/question-forecast-seeds.mjs \
  --limit 5
node scripts/kgab-dossier-bridge.mjs \
  --auto-clone \
  --topic 공급망 \
  --policy-query 조달청 \
  --law-query 정부조직법
node scripts/gazette-readable-bridge.mjs \
  ...
  --limit 5 \
  --fetch-readable
node scripts/gov-press-md-bridge.mjs \
  --keyword 공급망 \
  --date 2025-05-16 \
  --limit 5 \
  --fetch-readable
node scripts/upstream-evidence-pack.mjs \
  --topic 공급망 \
  --policy-query 조달청 \
  --law-query 정부조직법 \
  --gazette-query 공급망 \
