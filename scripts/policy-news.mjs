...
async function main() {
  ...
  const items = blocks(xml, "item").map(b => ({
    title: tag(b, "title"),
    date: tag(b, "regDate") || tag(b, "date"),
    agency: tag(b, "deptName") || tag(b, "department"),
    summary: tag(b, "contents") || tag(b, "summary"),
    source_url: tag(b, "link") || tag(b, "url"),
    raw: b,
  }));
  const payload = { metadata: { source: "정책브리핑 정책뉴스 API", strategy: "KEYED_API_XML", retrieved_at: new Date().toISOString(), query_url: redact(url.toString()), count: items.length }, items };
